...
if __name__ == '__main__':
    ...
    for i, source in enumerate(sources, start=1):
        ...
        if scaling == 'pdf' and powerlaw_index_lim and \
            ...
            print(f'Power law index > {powerlaw_index_lim}. Skip source for ' \
                  'now.')
            continue
        ...
        n_todo = n_sim - n_done
        print(f'{n_todo} light curves stored.')

        if n_todo > 0:
            print(f'{n_todo} more light curves will be simulated..\n')
        else:
        ...
        # set spectral shape:
        spec_shape = 'powerlaw'
        # BUGFIX NEEDED: Regards line above. I could also write `sim.powerlaw`
        # instead of `'powerlaw'`, because a callable function works for the
        # simulation code; however this will crash when writing to the
        # database, which expects a string.
        spec_args = (powerlaw_index, 10, 10**(-7))
        # note regarding line above: power of 10. at 10^-7 gives a fairly
        # stable LC amplitude scale, independent of index
        ...
        while n_todo > 0:
            ...
            if scaling == 'pdf':
                n_created = sim.sim_emp(
                    spec_shape, spec_args, pdf, pdf_params=None,
                    pdf_range=None, nlcs=nlcs, iterations=n_iter_pdf,
            ...
            for lc in sim.iter_lcs():
                ...
                # resample artificial light curves, add observational errors:
